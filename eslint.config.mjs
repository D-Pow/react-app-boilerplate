...
export default tseslint.config([
    ...
    ReactPlugin.configs['recommended-typescript'],
    {
        languageOptions: {
            ...
            parserOptions: {
                ...
                // project: tsconfigDevPath, // tsconfig file (or array of files) from which to extract. Included automatically with `tseslint.config()`
                // projectService: {
        ...
        plugins: {
            // No longer needs `react` as a plugin due to adding it to flatConfig array above
        ...
        rules: {
            ...
            '@stylistic/jsx-equals-spacing': 'error', // Prevent spaces between JSX prop names and equals sign, e.g. `<MyComp prop = {value} />`
            '@eslint-react/no-missing-component-display-name': 'off', // Don't error on arrow-function components
            '@eslint-react/rules-of-hooks': 'error', // Enforce proper usage of hooks
            '@eslint-react/exhaustive-deps': 'warn', // Suggest adding all fields within hook dependencies, e.g. `useEffect(func, [ dep1 ])` errors if `dep2` left out
            // TODO: Wait until there is a `@stylistic/eslint-plugin` or `@eslint-react/eslint-plugin` rule that is equivalent to `react/jsx-filename-extension`
     ...
     */
    {
        files: [ '**/?(.)+(*.)?([mc])js?(x)' ],
        rules: {
            // Some CJS files still use `require()` for compatibility. Since we are wrapping everything in
            // `typescript-eslint`, we need to manually disable this rule for configs, test setup, etc.
    ...
    /* Tests */
    {
        ...
        languageOptions: {
            globals: {
                ...globals.node,
                ...globals.jest, // Add jest environment for tests
            },
        },
    },
]);
